const formats = [
  {
    emoji: "⚡",
    title: "Книга в одном предложении",
    platforms: ["Twitter", "Telegram", "TikTok"],
    why: "Узнаёшь книгу за 3 секунды, смешно пересказываешь другу.",
    example: "«Анна Каренина»: женщина хотела жить по-своему — эпоха не одобрила.",
  },
  {
    emoji: "📱",
    title: "Если бы писатели вели Telegram-каналы",
    platforms: ["Instagram", "VK"],
    why: "Достоевский — канал «Преступление и подписка». Чехов — «Жалобы на пациентов».",
    example: "Толстой: «Сегодня написал 80 страниц. Завтра вычеркну 79».",
  },
  {
    emoji: "🎬",
    title: "Школьная программа: пересдаём эмоциями",
    platforms: ["Reels", "TikTok"],
    why: "15 секунд на камеру — и вся боль от «Войны и мира» понятна без слов.",
    example: "«Война и мир»: 1200 страниц, чтобы понять — люди тупые всегда.",
  },
  {
    emoji: "🤡",
    title: "Ожидание / Реальность",
    platforms: ["Pinterest", "Reddit", "VK"],
    why: "Ожидание: «буду читать умное». Реальность: уснул на 3 странице.",
    example: "Ожидание: Пруст изменит жизнь. Реальность: 3000 страниц про печенье.",
  },
  {
    emoji: "💥",
    title: "Три книги, которые тебя сломали",
    platforms: ["Stories", "Опросы"],
    why: "«Над пропастью во ржи», «451°», «Цветы для Элджернона» — люди отвечают массово.",
    example: "Челлендж запускает волну — каждый вспоминает свою личную травму.",
  },
];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col gap-px bg-neutral-200">
          {formats.map((f, i) => (
            <div
              key={f.title}
              className="bg-neutral-50 hover:bg-white transition-colors duration-300 group"
            >
              <div className="grid grid-cols-[3rem_1fr] md:grid-cols-[3rem_1fr_1fr_1fr] gap-4 md:gap-8 items-start p-6 md:p-8">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {f.platforms.map((p) => (
                      <span
                        key={p}
                        className={`text-[10px] uppercase tracking-widest px-2 py-0.5 ${platformColors[p] ?? "bg-neutral-100 text-neutral-600"}`}
                      >
                        {p}
                      </span>
                    ))}
                ...
                <p className="hidden md:block text-neutral-400 text-sm italic leading-relaxed border-l border-neutral-200 pl-4">
                  {f.example}
                </p>
